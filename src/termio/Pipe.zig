...
//! be fed externally by the host through the embedded pipe read API.
const Pipe = @This();
...
test "initTerminal sends initial resize" {
    const Ctx = struct {
        called: bool = false,
        grid: renderer.GridSize = undefined,
        screen: renderer.ScreenSize = undefined,
    };

    const callbacks = struct {
        fn resize(userdata: ?*anyopaque, grid: renderer.GridSize, screen: renderer.ScreenSize) void {
            const ctx: *Ctx = @ptrCast(@alignCast(userdata orelse return));
            ctx.called = true;
            ctx.grid = grid;
            ctx.screen = screen;
    ...
    try std.testing.expectEqual(@as(u16, 80), ctx.grid.columns);
    try std.testing.expectEqual(@as(u16, 24), ctx.grid.rows);
    try std.testing.expectEqual(@as(u32, 800), ctx.screen.width);
    try std.testing.expectEqual(@as(u32, 480), ctx.screen.height);
}

test "resize without callback is a no-op" {
    var pipe = init(.{});
    try pipe.resize(.{
        .columns = 1,
        .rows = 1,
